...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviourPun
	{
 		...
 		private void Awake()
		{
			...
			{
				playerControllerParent.tag = "Player";
			}
		}
		...
		[Header("Gaze Settings")]
 		public float objectRange=3.5f;
		public GazeData GazeData;
		public IHoverable _current;
		public Transform holderReference;
		public static Transform holder;
		public GazeObject gazeObject;
		public GameObject playerControllerParent;
		public   bool isSomethingPicked;
		public void GazePointer()
		{
			...
			{
				next = hit.collider.GetComponent<IHoverable>();
 			}
			var data = this.GazeData;
			data.GazeScript = this;
			data.ViewCamera = Camera.main;

			 
				if (_current != null && _current != next)
				{
				 
					_current.OnGazeExit(data);
					//_current.SetLooker(null);
 					_current = null;
				...
				{
					_current = next;
				_current.SetLooker(this.gameObject);

				_current.OnGazeEnter(data);
				...
				if (_current != null && _input.pickUp)
				{
					//this.photonView.TransferOwnership(hold.gameObject.GetComponent<PhotonView>().Owner);
					_current.OnClick(data);
